...
(function () {
  ...
  window.nova_plus_tor = true;

  var TOR_ON_KEY = 'nova_plus_enabled';
  var TOR_KEY = 'nova_plus_tor_skin';
  var TOR_STYLE = 'nova-plus-tor-style';
  var TOR_SCOPE = 'nova-plus-tor';

  var tor_timer = null;
  var tor_observer = null;

  var TOR_OWN = {
    nova_plus_set_tor: {
      ru: 'Страница торрентов',
      uk: 'Сторінка торентів',
      en: 'Torrents page'
    },
  ...
  var TOR_CSS = [
    ...
    '.explorer.nova-plus-tor .explorer__left{-webkit-flex:0 0 41%!important;-ms-flex:0 0 41%!important;flex:0 0 41%!important;width:41%!important;max-width:41%!important;padding:0 1.6em 0 0!important}',
    ...
    '.explorer.nova-plus-tor .nova-tor-hero__title--logo{width:100%!important;max-width:100%!important;text-align:center!important}',
      ...
      if (list && list.length) {
        ...
        if (!file) {
          for (var j = 0; j < list.length; j++) {
            if (list[j] && list[j].iso_639_1 === 'en' && list[j].file_path) { file = list[j].file_path; break; }
          }
        }
        if (!file && list[0] && list[0].file_path) file = list[0].file_path;
      }
      tor_logo_cache[key] = file || '';
      done(tor_logo_cache[key]);
    }, function () {
      tor_logo_cache[key] = '';
      done('');
    });
  ...
  function torLogoDraw(slot, file, title) {
    if (!slot || !slot.length) return;
    if (!file) {
      slot.removeClass('nova-tor-hero__title--logo').text(title);
      return;
    }
    var src = '';
    try { src = Lampa.TMDB.image('t/p/w780' + String(file).replace('.svg', '.png')); } catch (e) { src = ''; }
    if (!src) {
      slot.removeClass('nova-tor-hero__title--logo').text(title);
      return;
    }
    ...
    img.attr('src', src);
    slot.addClass('nova-tor-hero__title--logo').empty().append(img);
  }

  function torRoot() {
    var act = torActive();
    if (!act || act.component !== 'torrents') return null;
    var box = null;
    try { box = $('.activity--active').find('.explorer'); } catch (e) { box = null; }
    if (!box || !box.length) {
      try { box = $('.explorer'); } catch (e) { box = null; }
    }
    return box && box.length ? box.eq(0) : null;
  }

  function torHero(box, movie) {
    ...
    if (hero.attr('data-sign') !== sign) {
      ...
      var titleSlot = hero.find('.nova-tor-hero__title');
      titleSlot.text(title);

      torLogoFetch(movie, function (logoFile) {
        if (!hero.parent().length) return;
        torLogoDraw(titleSlot, logoFile, title);
      });
    ...
    var img = hero.find('.nova-tor-hero__bg img')[0];

    if (art && img && img.getAttribute('data-art') !== art) {
      img.setAttribute('data-art', art);
      img.onload = function () { hero.addClass('nova-tor-hero--loaded'); };
      img.onerror = function () { hero.removeClass('nova-tor-hero--loaded'); };
      img.src = art;
    }
  }
  ...
  function torSoon(wait) {
    clearTimeout(tor_timer);
    tor_timer = setTimeout(torApply, wait || 60);
  }

  function torStop() {
    try { if (tor_observer) tor_observer.disconnect(); } catch (e) {}
    tor_observer = null;
    clearTimeout(tor_timer);
    tor_timer = null;
  }
  ...
  function torWatch() {
    ...
    try {
      tor_observer = new MutationObserver(function () { torSoon(60); });
      tor_observer.observe(box[0], { childList: true, subtree: true });
    } catch (e) {}
  }
  ...
  function torSettings() {
    try {
      Lampa.SettingsApi.addParam({
        component: 'nova_plus',
        param: { name: TOR_KEY, type: 'trigger', default: true },
        field: {
          name: torText('nova_plus_set_tor'),
          description: torText('nova_plus_set_tor_descr')
        },
        onChange: function () { try { Lampa.Activity.replace(); } catch (e) {} }
      });
    } catch (e) {}
