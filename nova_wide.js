(function () {
  ...
  if (window.nova_wide) return;
  window.nova_wide = true;

  var ENABLED_KEY = 'nova_wide_enabled';
  var JUMP_FROM = 20;
  var SEEN_PERCENT = 90;
  var LOGO_DARK = 0.4;
  ...
  var LOGO_MEM = {};
  var LOGO_CACHE_KEY = 'nova_wide_logo_cache2';
  ...
  function heroEnabled() { return true; }
  function artEnabled() { return true; }
  function preferredQuality() { return get('nova_wide_quality', 'auto'); }

  function focusRing() { return get('nova_wide_focus_style', 'ring') !== 'fill'; }

  function fullScreen() { return true; }

  function applyFullScreen() {
    try {
      var body = $('body');
      if (fullScreen()) body.addClass('nova-wide-full');
      else body.removeClass('nova-wide-full');
    } catch (e) {}
  }

  function edgeFade() { return get('nova_wide_fade', false) === true; }

  function applyEdgeFade() {
    try {
      var body = $('body');
      if (edgeFade()) body.addClass('nova-wide-fade');
      else body.removeClass('nova-wide-fade');
    } catch (e) {}
  }
  ...
  function applyFocusStyle() {
    try {
      var body = $('body');
      if (focusRing()) body.addClass('nova-wide-focus-ring');
      else body.removeClass('nova-wide-focus-ring');
    } catch (e) {}
  }

  var OWN = {
    nova_wide_watch: { ru: 'Смотреть', uk: 'Дивитися', en: 'Watch' },
    nova_wide_continue: { ru: 'Продолжить', uk: 'Продовжити', en: 'Continue' },
    nova_wide_from_start: { ru: 'Смотреть с начала', uk: 'Дивитися з початку', en: 'Watch from start' },
    nova_wide_next_episode: { ru: 'Следующая', uk: 'Наступна', en: 'Next' },
    nova_wide_first_new: { ru: 'Первая непросмотренная', uk: 'Перша непроглянута', en: 'First unwatched' },
    nova_wide_source: { ru: 'Источник', uk: 'Джерело', en: 'Source' },
    nova_wide_season: { ru: 'Сезон', uk: 'Сезон', en: 'Season' },
    nova_wide_voice: { ru: 'Перевод', uk: 'Переклад', en: 'Audio' },
    nova_wide_jump: { ru: 'Серии', uk: 'Серії', en: 'Episodes' },
    nova_wide_jump_pick: { ru: 'Выбрать серию', uk: 'Вибрати серію', en: 'Pick an episode' },
    nova_wide_files: { ru: 'Файлы', uk: 'Файли', en: 'Files' },
    nova_wide_playlist: { ru: 'Плейлист', uk: 'Плейлист', en: 'Playlist' },
    nova_wide_files_pick: { ru: 'Выбрать файл', uk: 'Вибрати файл', en: 'Pick a file' },
    nova_wide_clarify: { ru: 'Уточнить поиск', uk: 'Уточнити пошук', en: 'Refine search' },
    nova_wide_more_sources: { ru: 'Ещё {count}', uk: 'Ще {count}', en: '{count} more' },
    nova_wide_season_progress: { ru: 'Просмотрено {seen} из {total}', uk: 'Переглянуто {seen} з {total}', en: 'Watched {seen} of {total}' },
    nova_wide_season_left: { ru: 'осталось {left}', uk: 'залишилось {left}', en: '{left} left' },
    nova_wide_season_planned: { ru: 'всего в сезоне {planned}', uk: 'усього в сезоні {planned}', en: '{planned} in the season' },
    nova_wide_left: { ru: 'осталось', uk: 'залишилось', en: 'left' },
    nova_wide_loading_title: { ru: 'Ищем, где посмотреть', uk: 'Шукаємо, де подивитися', en: 'Looking for a source' },
    nova_wide_loading_start: { ru: 'Опрашиваем источники', uk: 'Опитуємо джерела', en: 'Checking sources' },
    nova_wide_sec: { ru: ' с', uk: ' с', en: ' s' },
    nova_wide_episode: { ru: 'Серия', uk: 'Серія', en: 'Episode' },
    nova_wide_action: { ru: 'Действие', uk: 'Дія', en: 'Action' },
    nova_wide_unknown: { ru: 'Неизвестно', uk: 'Невідомо', en: 'Unknown' },
    nova_wide_voice_dub: { ru: 'Дубляж', uk: 'Дубляж', en: 'Dubbed' },
    nova_wide_voice_mvo: { ru: 'Многоголосый', uk: 'Багатоголосий', en: 'Multi-voice' },
    nova_wide_voice_dvo: { ru: 'Двухголосый', uk: 'Двоголосий', en: 'Two-voice' },
    nova_wide_voice_avo: { ru: 'Авторский', uk: 'Авторський', en: 'Single-voice' },
    nova_wide_voice_orig: { ru: 'Оригинал', uk: 'Оригінал', en: 'Original' },
    nova_wide_voice_sub: { ru: 'Субтитры', uk: 'Субтитри', en: 'Subtitles' },
    nova_wide_voice_other: { ru: 'Другое', uk: 'Інше', en: 'Other' },
    nova_wide_try_source: { ru: 'Попробовать {name}', uk: 'Спробувати {name}', en: 'Try {name}' },
    nova_wide_all_sources: { ru: 'Все источники', uk: 'Всі джерела', en: 'All sources' },
    nova_wide_retry: { ru: 'Повторить', uk: 'Повторити', en: 'Retry' },
    nova_wide_auto_next: { ru: 'Через {sec} сек переключимся на «{name}»', uk: 'Через {sec} с перейдемо на «{name}»', en: 'Switching to "{name}" in {sec}s' },
    nova_wide_dead_all: { ru: 'Ни один источник ничего не нашёл', uk: 'Жодне джерело нічого не знайшло', en: 'No source found anything' },
    nova_wide_set_enable: { ru: 'Включить Nova Wide', uk: 'Увімкнути Nova Wide', en: 'Enable Nova Wide' },
    nova_wide_set_enable_descr: { ru: 'Единый интерфейс для всех онлайн-плагинов', uk: 'Єдиний інтерфейс для всіх онлайн-плагінів', en: 'One interface for every online plugin' },
    nova_wide_set_logo: { ru: 'Логотип вместо названия', uk: 'Логотип замість назви', en: 'Logo instead of title' },
    nova_wide_set_art_size: { ru: 'Качество кадра', uk: 'Якість кадру', en: 'Backdrop quality' },
    nova_wide_set_art_size_descr: { ru: 'Разрешение картинки в шапке. Выше — резче, но тяжелее', uk: 'Роздільна здатність картинки в шапці. Вище — різкіше, але важче', en: 'Header artwork resolution. Higher is sharper but heavier' },
    nova_wide_art_auto: { ru: 'Авто (по экрану)', uk: 'Авто (за екраном)', en: 'Auto (by screen)' },
    nova_wide_art_780: { ru: 'Обычное — 780px', uk: 'Звичайна — 780px', en: 'Normal — 780px' },
    nova_wide_art_1280: { ru: 'Высокое — 1280px', uk: 'Висока — 1280px', en: 'High — 1280px' },
    nova_wide_art_orig: { ru: 'Максимальное (тяжёлое)', uk: 'Максимальна (важка)', en: 'Maximum (heavy)' },
    nova_wide_set_logo_descr: { ru: 'Показывать логотип фильма в шапке, если он есть', uk: 'Показувати логотип фільму в шапці, якщо він є', en: 'Show the movie logo in the header when available' },
    nova_wide_set_fade: { ru: 'Размытые края постера', uk: 'Розмиті краї постера', en: 'Faded poster edges' },
    nova_wide_set_fade_descr: { ru: 'Растворять постер вверху по краям со всех сторон', uk: 'Розчиняти постер угорі по краях з усіх боків', en: 'Fade the header artwork out on every side' },
    nova_wide_set_probe: { ru: 'Проверять источники в фоне', uk: 'Перевіряти джерела у фоні', en: 'Check sources in background' },
    nova_wide_set_probe_descr: { ru: 'Отмечать рабочие точкой и показывать их качество', uk: 'Позначати робочі точкою та показувати їхню якість', en: 'Mark working ones with a dot and show their quality' },
    nova_wide_set_probe_ext: { ru: 'Источники проверяет сам онлайн-плагин, повторный обход не нужен', uk: 'Джерела перевіряє сам онлайн-плагін, повторний обхід не потрібен', en: 'The online plugin checks sources itself, no second pass needed' },
    nova_wide_set_switch: { ru: 'Автопереход по источникам', uk: 'Автоперехід по джерелах', en: 'Auto switch source' },
    nova_wide_set_switch_descr: { ru: 'Если ничего не найдено, пробовать следующий рабочий источник', uk: 'Якщо нічого не знайдено, пробувати наступне робоче джерело', en: 'Try the next working source when nothing is found' },
    nova_wide_set_quality: { ru: 'Качество по умолчанию', uk: 'Якість за замовчуванням', en: 'Default quality' },
    nova_wide_set_quality_descr: { ru: 'Предпочтительное качество воспроизведения', uk: 'Бажана якість відтворення', en: 'Preferred playback quality' },
    nova_wide_set_quality_auto: { ru: 'Авто', uk: 'Авто', en: 'Auto' },
    nova_wide_set_focus: { ru: 'Выделение', uk: 'Виділення', en: 'Highlight style' },
    nova_wide_set_focus_descr: { ru: 'Чем подсвечивать выбранную кнопку, серию или озвучку', uk: 'Чим підсвічувати вибрану кнопку, серію або озвучення', en: 'How the focused button, episode or voice is highlighted' },
    nova_wide_set_focus_ring: { ru: 'Ободок', uk: 'Обідок', en: 'Outline' },
    nova_wide_set_focus_fill: { ru: 'Белая заливка', uk: 'Біла заливка', en: 'White fill' }
  };
  ...
  var VOICE_KINDS = [
    { key: 'dub', own: 'nova_wide_voice_dub', title: 'nova_voice_dub', re: /дубляж|дублирован|\bdub\b|\bdubbing\b/i },
    { key: 'mvo', own: 'nova_wide_voice_mvo', title: 'nova_voice_mvo', re: /многоголос|\bmvo\b|\bpmvo\b/i },
    { key: 'dvo', own: 'nova_wide_voice_dvo', title: 'nova_voice_dvo', re: /двухголос|\bdvo\b/i },
    { key: 'avo', own: 'nova_wide_voice_avo', title: 'nova_voice_avo', re: /авторск|одноголос|\bavo\b|\bvo\b/i },
    { key: 'orig', own: 'nova_wide_voice_orig', title: 'nova_voice_orig', re: /оригинал|original|\beng\b|\bua\b|\bukr\b/i },
    { key: 'sub', own: 'nova_wide_voice_sub', title: 'nova_voice_sub', re: /субтитр|sub(title)?s?\b/i }
  ];
  ...
  function addCSS() {
    if (document.getElementById('nova-wide-css')) return;
    var style = document.createElement('style');
    style.id = 'nova-wide-css';
  ...
  function hookFilter() {
    if (!Lampa.Filter || Lampa.Filter.nova_wide_wrapped) return;
    ...
    Wrapped.nova_wide_wrapped = true;
    for (var key in real) Wrapped[key] = real[key];
    Lampa.Filter = Wrapped;
  }

  function hookSelect() {
    if (!Lampa.Select || Lampa.Select.nova_wide_wrapped) return;
    ...
    Lampa.Select.nova_wide_wrapped = true;
  }
  ...
  function hookRequest() {
    if (!Lampa.Reguest || Lampa.Reguest.nova_wide_wrapped) return;
  ...
  function hookXHR() {
    try {
      if (!window.XMLHttpRequest || XMLHttpRequest.prototype.nova_wide_wrapped) return;
      ...
      XMLHttpRequest.prototype.nova_wide_wrapped = true;
    } catch (e) {}
  }

  function hookScroll() {
    if (!Lampa.Scroll || Lampa.Scroll.nova_wide_wrapped) return;
  ...
  function hookController() {
    if (!Lampa.Controller || Lampa.Controller.nova_wide_wrapped) return;
    var add = Lampa.Controller.add;

    Lampa.Controller.add = function (name, object) {
      if (name === 'content' && object && !object.nova_wide_hooked) {
        object.nova_wide_hooked = true;
    ...
    Lampa.Controller.nova_wide_wrapped = true;
  }
  ...
  function readExtras() {
    ...
    head.find('.simple-button.selector').each(function () {
      var node = $(this);
      if (node.is(KNOWN_BUTTONS) || node.closest('.nova-wide-root').length) return;
  ...
  function artSize() {
    var want = String(get('nova_wide_art_size', 'auto') || 'auto');
  ...
  function dropStale() {
    ...
    $(host).find('.nova-wide-root').each(function () {
      if (this === mine) return;
  ...
  function uiFrame() {
    ...
    if (!ui.root) {
      ui.root = $('<div class="nova nova-wide-root"></div>');
      ui.hero_box = $('<div class="nova__hero"></div>');
      ui.rows = $('<div class="nova__rows"></div>');
      ui.list = $('<div class="nova__list"></div>');
      ui.wide = wideOn();
      if (ui.wide) {
        ui.root.addClass('nova-wide');
        ui.strip = $('<div class="nova-wide__strip"></div>').append(ui.list);
  ...
  function autoSwitchOn() {
    return get('nova_wide_auto_switch', true) !== false;
  }
  ...
  function probeOn() {
    return get('nova_wide_probe', false) === true;
  }
  ...
  function hookReplace() {
    if (!Lampa.Activity || Lampa.Activity.nova_wide_wrapped) return;
    ...
    Lampa.Activity.nova_wide_wrapped = true;
  }
  ...
  function hideHost() {
    ...
    $(host).children().each(function () {
      ...
      $(this).addClass('nova-wide-hidden');
    });
  }
  ...
  function reattach() {
    ...
    root.addClass('nova-wide-scope nova-wide-chips');
  ...
  function collect() {
    ...
    $(host).find('.online-prestige--full,.online-prestige--folder').each(function () {
      if ($(this).closest('.nova-wide-root').length) return;
  ...
  function seasonText(season) {
    var line = text('nova_season_progress', 'nova_wide_season_progress')
      .replace('{seen}', season.seen).replace('{total}', season.total);
    if (season.seen < season.total) {
      line += ' \u00b7 ' + text('nova_season_left', 'nova_wide_season_left')
        .replace('{left}', season.total - season.seen);
    }
    if (season.planned > season.total) {
      line += ' \u00b7 ' + text('nova_season_planned', 'nova_wide_season_planned')
  ...
  function buildCard(item, compact, grid) {
    ...
    if (item.percent > 0 && item.percent < SEEN_PERCENT && item.time) {
      ...
      if (left > 0) meta.push(text('nova_left', 'nova_wide_left') + ' ' + runtimeText(left));
    }
  ...
  function episodeSuffix(item) {
    ...
    return ' \u00b7 ' + text('torrent_serial_episode', 'nova_wide_episode') + ' ' + item.num;
  }
  ...
  function playMenu() {
    ...
    var menu = [{
      title: (started ? text('nova_continue', 'nova_wide_continue') : text('nova_watch', 'nova_wide_watch')) + episodeSuffix(target),
      item: target
    }];

    if (started) {
      menu.push({
        title: text('nova_from_start', 'nova_wide_from_start') + episodeSuffix(target),
        item: target
      });
    }
    ...
    if (next) {
      menu.push({
        title: text('nova_next_episode', 'nova_wide_next_episode') + episodeSuffix(next),
        item: next
      });
    }
    ...
    if (fresh && fresh !== target && fresh !== next) {
      menu.push({
        title: text('nova_first_new', 'nova_wide_first_new') + episodeSuffix(fresh),
        item: fresh
      });
    }
    ...
    if (items.length > JUMP_FROM) {
      menu.push({
        title: serial
          ? text('nova_jump_pick', 'nova_wide_jump_pick')
          : text('nova_files_pick', 'nova_wide_files_pick'),
        jump: true
      });
    }
    ...
    try {
      Lampa.Select.show({
        title: text('title_action', 'nova_wide_action'),
        items: menu,
        onBack: function () {
  ...
  function logoOn() {
    return get('nova_wide_logo', true) !== false;
  }
  ...
  function logoEntry(key) {
    ...
    var old = cached('nova_wide_logo_cache', 500, {});
    var was = old && old[key];
  ...
  function seasonPlanned(season, done) {
    ...
    var key = id + ':' + season;
    var box = cached('nova_wide_season_cache', 300, {});
    ...
    var keep = function (count) {
      var now = cached('nova_wide_season_cache', 300, {});
      if (now && typeof now === 'object') {
        now[key] = count || 0;
        save('nova_wide_season_cache', now);
      }
      done(count || 0);
    };
  ...
  function buildHero() {
    ...
    if (next && started) {
      next_button.find('.nova-btn__label')
        .text(text('nova_next_episode', 'nova_wide_next_episode') + episodeSuffix(next));
    ...
    var label = started ? text('nova_continue', 'nova_wide_continue') : text('nova_watch', 'nova_wide_watch');
  ...
  function sourceRow() {
    ...
    if (!ui_all_sources && hidden.length) {
      var more = chip('src:more', text('nova_more_sources', 'nova_wide_more_sources')
  ...
  function buildRows() {
    ...
    var toolbar = $('<div class="nova-toolbar nova-wide__bar"></div>');
    chip_actions = {};
    ...
    if (groups.season && groups.season.items.length > 1) {
      if (serial) {
        addChip('season', groups.season.title || text('torrent_serial_season', 'nova_wide_season'),
          groups.season.subtitle || '', {});
      } else {
        addChip('season', text('nova_playlist', 'nova_wide_playlist'),
          partTitle(groups.season.subtitle), {});
      }
    }

    if (groups.voice && groups.voice.items.length > 1) {
      addChip('voice', groups.voice.title || text('torrent_parser_voice', 'nova_wide_voice'),
    ...
    if (!nav && items.length > JUMP_FROM) {
      ...
      addChip('jump', serial
        ? text('nova_jump', 'nova_wide_jump')
        : text('nova_files', 'nova_wide_files'), pageTitle(now), {});
    }
  ...
  function nativeState() {
    var empty = $(host).find('.online-empty').not('.nova-wide-root .online-empty').first();
  ...
  var KNOWN_KEY = 'nova_wide_components';
  ...
  function blank() {
    ...
    var body = $(host).children().not('.nova-wide-root').not('.nova-wide-hidden');
  ...
  function loadingPanel() {
    ...
    if (!ui.load) {
      ...
      ui.load = $('<div class="nova-loading">' +
        ...
        '</div>');
      ui.load.find('.nova-loading__title').text(text('nova_loading_title', 'nova_wide_loading_title'));
    }

    listHold().empty().append(ui.load).append(skeleton(10));
    loadingText();
  ...
  function loadingText() {
    ...
    var line = text('nova_loading_start', 'nova_wide_loading_start') +
      ' \u00b7 ' + seconds + text('nova_sec', 'nova_wide_sec');
  ...
  function notePanel(native) {
    ...
    if (next) {
      addAction(text('nova_try_source', 'nova_wide_try_source')
    ...
    if ((groups.sort || []).length > 1) {
      addAction(text('nova_all_sources', 'nova_wide_all_sources'), ICON.chevron, function () {
        uiToggle('source');
      }, true);
    }

    addAction(text('nova_retry', 'nova_wide_retry'), ICON.refresh, function () {
      var comp = componentNow();
      if (reloadable(comp)) {
        signature = '';
        listHold().empty().append(skeleton(4));
    ...
    if (auto) {
      ...
      var render = function () {
        slot.text(text('nova_auto_next', 'nova_wide_auto_next')
      ...
      note.find('.nova-note__text').text(text('nova_dead_all', 'nova_wide_dead_all'));
    }
    ...
    if (search.length) {
      ...
      clarify.append(ICON.search).append($('<span></span>').text(text('nova_clarify', 'nova_wide_clarify')));
  ...
  function draw() {
    ...
    if (native) {
      ...
      knownRemember();
      root.addClass('nova-wide-scope nova-wide-chips');
      native.node.addClass('nova-wide-hidden');
      items = [];
      signature = '';
  ...
  function scoped() {
    return !!(root && root.hasClass('nova-wide-scope'));
  }
  ...
  function attach() {
    ...
    observer = new MutationObserver(function (records) {
      for (var i = 0; i < records.length; i++) {
        ...
        if (node && node.nodeType === 1 && $(node).closest('.nova-wide-root').length) continue;
  ...
  function settings() {
    try {
      ...
      Lampa.SettingsApi.addParam({
        ...
        field: {
          name: label('nova_wide_set_enable'),
          description: label('nova_wide_set_enable_descr')
        },
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_wide',
        param: {
          name: 'nova_wide_art_size',
          type: 'select',
          values: {
            auto: label('nova_wide_art_auto'),
            w780: label('nova_wide_art_780'),
            w1280: label('nova_wide_art_1280'),
            original: label('nova_wide_art_orig')
          },
          default: 'auto'
        },
        field: {
          name: label('nova_wide_set_art_size'),
          description: label('nova_wide_set_art_size_descr')
        },
        onChange: function () { try { Lampa.Activity.replace(); } catch (e) {} }
      });
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_wide',
        param: { name: 'nova_wide_logo', type: 'trigger', default: true },
        field: {
          name: label('nova_wide_set_logo'),
          description: label('nova_wide_set_logo_descr')
        },
        onChange: function () {
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_wide',
        param: { name: 'nova_wide_fade', type: 'trigger', default: false },
        field: {
          name: label('nova_wide_set_fade'),
          description: label('nova_wide_set_fade_descr')
        },
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_wide',
        param: { name: 'nova_wide_probe', type: 'trigger', default: false },
        field: {
          name: label('nova_wide_set_probe'),
          description: label('nova_wide_set_probe_descr')
        },
      ...
      try {
        Lampa.Settings.listener.follow('open', function (e) {
          if (!e || e.name !== 'nova_wide' || !e.body) return;

          var mode = probeHook();
          var item = e.body.find('[data-name="nova_wide_probe"]');
          ...
          if (mode === 'external') {
            descr.text(label('nova_wide_set_probe_ext'));
            item.css('opacity', '.6');
          } else {
            descr.text(label('nova_wide_set_probe_descr'));
            item.css('opacity', '');
          }
        });
      } catch (e) {}
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_wide',
        param: { name: 'nova_wide_auto_switch', type: 'trigger', default: true },
        field: {
          name: label('nova_wide_set_switch'),
          description: label('nova_wide_set_switch_descr')
        }
      });
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_wide',
        param: {
          name: 'nova_wide_quality',
          type: 'select',
          values: {
            auto: label('nova_wide_set_quality_auto'),
            2160: '4K',
            1080: '1080p',
            720: '720p',
            480: '480p'
        ...
        field: {
          name: label('nova_wide_set_quality'),
          description: label('nova_wide_set_quality_descr')
        }
      });
      ...
      Lampa.SettingsApi.addParam({
        component: 'nova_wide',
        param: {
          name: 'nova_wide_focus_style',
          type: 'select',
          values: {
            ring: label('nova_wide_set_focus_ring'),
            fill: label('nova_wide_set_focus_fill')
          },
          default: 'ring'
        },
        field: {
          name: label('nova_wide_set_focus'),
          description: label('nova_wide_set_focus_descr')
        },
  ...
  var WIDE_LAYOUT_KEY = 'nova_wide_layout';
  ...
  var WIDE_OWN = {
    nova_wide_seasons: { ru: 'Сезон', uk: 'Сезон', en: 'Season' },
    nova_wide_parts: { ru: 'Части', uk: 'Частини', en: 'Parts' },
    nova_wide_voices: { ru: 'Перевод', uk: 'Переклад', en: 'Audio' },
    nova_wide_ep_one: { ru: '{count} эпизод', uk: '{count} епізод', en: '{count} episode' },
    nova_wide_ep_few: { ru: '{count} эпизода', uk: '{count} епізоди', en: '{count} episodes' },
    nova_wide_ep_many: { ru: '{count} эпизодов', uk: '{count} епізодів', en: '{count} episodes' },
    nova_wide_all: { ru: 'всего {count}', uk: 'усього {count}', en: '{count} total' },
    nova_wide_order: { ru: 'Порядок', uk: 'Порядок', en: 'Order' },
    nova_wide_order_straight: { ru: 'Стандарт', uk: 'Стандарт', en: 'Standard' },
    nova_wide_order_reverse: { ru: 'Обратный', uk: 'Зворотний', en: 'Reversed' },
    nova_wide_look: { ru: 'Вид', uk: 'Вигляд', en: 'Layout' },
    nova_wide_look_row: { ru: 'Горизонтальный', uk: 'Горизонтальний', en: 'Horizontal' },
    nova_wide_look_grid: { ru: 'Вертикальный', uk: 'Вертикальний', en: 'Vertical' },
    nova_wide_reset: { ru: 'Сбросить фильтр', uk: 'Скинути фільтр', en: 'Reset filter' },
    nova_wide_set_pos: { ru: 'Положение источника', uk: 'Розташування джерела', en: 'Source row position' },
    nova_wide_set_pos_descr: { ru: 'Где стоит строка с источником: над сезонами или под ними', uk: 'Де стоїть рядок із джерелом: над сезонами або під ними', en: 'Where the source row sits: above or below the seasons' },
    nova_wide_pos_top: { ru: 'Вверху', uk: 'Вгорі', en: 'Top' },
    nova_wide_pos_bottom: { ru: 'Внизу', uk: 'Внизу', en: 'Bottom' },
    nova_wide_set_look: { ru: 'Лента серий', uk: 'Лента серій', en: 'Episode strip' },
    nova_wide_set_look_descr: { ru: 'Вертикальная сетка по умолчанию, горизонтальная по переключателю', uk: 'Вертикальна сітка за замовчуванням, горизонтальна за перемикачем', en: 'Vertical grid by default, horizontal by toggle' },
    nova_wide_set_order: { ru: 'Порядок серий', uk: 'Порядок серій', en: 'Episode order' },
    nova_wide_set_order_descr: { ru: 'Обратный удобен для сериалов, которые ещё выходят', uk: 'Зворотний зручний для серіалів, що ще виходять', en: 'Reversed suits shows that are still airing' },
    nova_wide_set_meta: { ru: 'Описания серий', uk: 'Описи серій', en: 'Episode details' },
    nova_wide_set_meta_descr: { ru: 'Добирать дату, рейтинг и описание серии из TMDB', uk: 'Добирати дату, рейтинг та опис серії з TMDB', en: 'Pull episode date, rating and overview from TMDB' }
  };
  ...
  var ALIEN_ROOT = '.nova-skin-root,.z01-root,.z01';
  var ALIEN_HIDDEN = '.nova-hidden,.z01-hidden';
  var ALIEN_SCOPE = 'nova-skin-scope nova-skin-chips z01-scope z01-chips';

  function wideEvict(body) {
    try {
      body.children('.nova-wide-root').removeClass('nova-hidden z01-hidden');
      ...
      alien.remove();
      body.find(ALIEN_HIDDEN).removeClass('nova-hidden z01-hidden');
  ...
  function wideCount(value) {
    ...
    var ten = count % 10;
    var key = 'nova_wide_ep_many';
    if (ten === 1 && hundred !== 11) key = 'nova_wide_ep_one';
    else if (ten >= 2 && ten <= 4 && (hundred < 10 || hundred >= 20)) key = 'nova_wide_ep_few';
    return label(key).replace('{count}', count);
  }

  function wideAll(value) {
    return label('nova_wide_all').replace('{count}', parseInt(value, 10) || 0);
  }
  ...
  function wideMark(key) {
    ...
    if (!box.length) return;
    box.closest('.nova-wide__row').find('.nova-chip--active').removeClass('nova-chip--active');
  ...
  function wideSourceChip() {
    if (!ui.rows) return null;
    var box = ui.rows.find('.nova-wide__bar [data-nova-focus="source"]').first();
  ...
  function wideTouch() {
    ...
    ['season', 'voice'].forEach(function (name) {
      ...
      group.items.forEach(function (item, seat) {
        ...
        if (name === 'season') {
          ...
          var shared = wideGroupCount();
          if (shared) box.find('.nova-chip__sub').text(wideCount(shared));
        }
      });
    });
    ...
    ui.rows.find('.nova-wide__bar .nova-chip').each(function () {
      var box = $(this);
  ...
  function wideRowFollow(node) {
    ...
    var row = $(node).closest('.nova-wide__row--scroll');
  ...
  function wideSeasonTotal() {
    ...
    var head = block.children('.nova-wide__label').first();
    if (!head.length) return;
    var slot = head.children('.nova-wide__total');
    var count = 0;
    ...
    if (!slot.length) slot = $('<span class="nova-wide__total"></span>').appendTo(head);
  ...
  function wideDragPane(box) {
    return box.children('.nova-wide__pane').first();
  }
  ...
  function wideSqueeze(row) {
    row.removeClass('nova-wide__row--pair').css('height', '');
  ...
  function wideFit() {
    ...
    var rows = panel.find('.nova-wide__group > .nova-wide__row');
    ...
    try {
      ...
      rows.each(function () {
        ...
        row.removeClass('nova-wide__row--scroll nova-wide__row--pair').css('height', '');
        wideRowShift(row, 0);
      });
      panel.find('.nova-wide__group').removeClass('nova-wide__group--scroll');
    } catch (e) { return; }
  ...
  function wideDropPane(drop) {
    var pane = drop.children('.nova-wide__pane').first();
    if (pane.length) return pane;
    pane = $('<div class="nova-wide__pane"></div>');
  ...
  function wideDropFit(drop) {
    ...
    drop.removeClass('nova-wide__drop--scroll').css('max-height', '');
    try {
      var hero = ui.hero_box[0].getBoundingClientRect();
  ...
  function widePaneFollow(node) {
    if (!node) return false;
    var drop = $(node).closest('.nova-wide__drop--scroll');
  ...
  function wideDrop() {
    ...
    panel.toggleClass('nova-wide__panel--note-source', wideSwapOn() && noteState);

    if (wideSwapOn() && !noteState) panel.addClass('nova-wide__panel--overlay');
  ...
  function wideRows() {
    ...
    if (wide_sig === sig && ui.rows.children('.nova-wide__panel').length) return wideTouch();
    wide_sig = sig;
    wideMark();

    ui.rows.empty();
    var panel = $('<div class="nova-wide__panel"></div>');
    ui.rows.append(panel);
    chip_actions = {};
    wide_hold = null;

    var bar = $('<div class="nova-toolbar nova-wide__bar"></div>');
    if (!widePosBottom()) panel.append(bar);
    panel.toggleClass('nova-wide__panel--bottom', widePosBottom());
    ...
    var group = function (title, name) {
      var block = $('<div class="nova-wide__group"></div>');
      if (name) block.attr('data-nova-group', name);
      if (title) block.append($('<div class="nova-wide__label"><span class="nova-wide__name"></span></div>').find('.nova-wide__name').text(title).end());
      var row = $('<div class="nova-wide__row"></div>');
      block.append(row);
      panel.append(block);
      return row;
    };
    ...
    if (groups.season && groups.season.items.length > 1) {
      var seasons = group(serial ? (groups.season.title || label('nova_wide_seasons')) : label('nova_wide_parts'), 'season');
    ...
    if (groups.voice && groups.voice.items.length > 1) {
      var voices = group(groups.voice.title || label('nova_wide_voices'), 'voice');
    ...
    if (serial && !nav) {
      tool('order', label('nova_wide_order'), label(wideReverse() ? 'nova_wide_order_reverse' : 'nova_wide_order_straight'), { plain: true }, function () {
    ...
    if (!nav) {
      tool('look', label('nova_wide_look'), label(wideRow() ? 'nova_wide_look_row' : 'nova_wide_look_grid'), { plain: true }, function () {
    ...
    if (wideResetable()) tool('reset', '', label('nova_wide_reset'), { plain: true }, wideReset);

    if (!nav && items.length > JUMP_FROM && !wideRow()) {
      var now = pageAt(pages(items.length), ui_page > 0 ? ui_page : 0);
      tool('jump', serial ? text('nova_jump', 'nova_wide_jump') : text('nova_files', 'nova_wide_files'), pageTitle(now), {}, function () { uiToggle('jump'); });
    }
  ...
  function wideSeen(node) {
    try {
      var clip = $(node).closest('.nova-wide__drop--scroll,.nova-wide__group--scroll');
  ...
  function wideStrip(list) {
    ...
    if (ui.list.hasClass('nova__list--row')) {
      ...
      strip.addClass('nova-wide__strip--clip');
    } else strip.removeClass('nova-wide__strip--clip');
    ui.list.attr('data-nova-x', 0);
    try { ui.list[0].style['-webkit-transform'] = ''; ui.list[0].style.transform = ''; } catch (e) {}
  }

  function wideSettings() {
    Lampa.SettingsApi.addParam({ component: 'nova_wide', param: { name: WIDE_LAYOUT_KEY, type: 'select', values: { grid: label('nova_wide_look_grid'), row: label('nova_wide_look_row') }, default: 'grid' }, field: { name: label('nova_wide_set_look'), description: label('nova_wide_set_look_descr') }, onChange: function () { redraw(); } });
    Lampa.SettingsApi.addParam({ component: 'nova_wide', param: { name: WIDE_ORDER_KEY, type: 'select', values: { straight: label('nova_wide_order_straight'), reverse: label('nova_wide_order_reverse') }, default: 'straight' }, field: { name: label('nova_wide_set_order'), description: label('nova_wide_set_order_descr') }, onChange: function () { redraw(); } });
    Lampa.SettingsApi.addParam({ component: 'nova_wide', param: { name: WIDE_META_KEY, type: 'trigger', default: true }, field: { name: label('nova_wide_set_meta'), description: label('nova_wide_set_meta_descr') }, onChange: function () { redraw(); } });
    Lampa.SettingsApi.addParam({ component: 'nova_wide', param: { name: WIDE_POS_KEY, type: 'select', values: { top: label('nova_wide_pos_top'), bottom: label('nova_wide_pos_bottom') }, default: 'top' }, field: { name: label('nova_wide_set_pos'), description: label('nova_wide_set_pos_descr') }, onChange: function () { redraw(); } });
  }

  var LOGO_CSS = ".nova-wide-root .nova-hero__title--logo>img.nova-logo--edge{-webkit-filter:drop-shadow(0 0 .02em rgba(255,255,255,.9)) drop-shadow(0 0 .06em rgba(255,255,255,.45)) drop-shadow(0 .04em .12em rgba(0,0,0,.5));filter:drop-shadow(0 0 .02em rgba(255,255,255,.9)) drop-shadow(0 0 .06em rgba(255,255,255,.45)) drop-shadow(0 .04em .12em rgba(0,0,0,.5))}";

  var LOGO_HOLD = '.nova-wide-root .nova-loading,.nova-wide-root .nova-note{width:100%;-webkit-box-flex:1;-webkit-flex:1 1 100%;-ms-flex:1 1 100%;flex:1 1 100%;min-width:0}'+
    '.nova-wide-root .nova-skeleton{width:100%;-webkit-box-flex:1;-webkit-flex:1 1 100%;-ms-flex:1 1 100%;flex:1 1 100%;min-width:0}'+
    '.nova-wide-root .nova-hero__title{min-height:2.3em}'+
    '.nova-wide-root .nova-hero__title--logo{display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-box-align:end;-webkit-align-items:flex-end;-ms-flex-align:end;align-items:flex-end;min-height:2.3em}'+
    '.nova-wide-root .nova-hero__mark.nova-hero__title--logo{min-height:0}'+
    '@media screen and (max-width:580px){.nova-wide-root .nova-hero__title{min-height:2.1em}.nova-wide-root .nova-hero__title--logo{min-height:2.1em}}'+
    '.nova-wide-root .nova-skeleton--grid{display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap;margin:0 -.45em}'+
    '.nova-wide-root .nova-skeleton--grid .nova-skeleton__row{display:block;width:25%;margin:0 0 1em 0;padding:0 .45em;background:none;-webkit-border-radius:0;border-radius:0}'+
    '.nova-wide-root .nova-skeleton--grid .nova-skeleton__thumb{width:100%;height:0;padding-top:56%;-webkit-border-radius:.5em;border-radius:.5em}'+
    '.nova-wide-root .nova-skeleton--grid .nova-skeleton__body{padding:.5em .1em 0 .1em}'+
    '.nova-wide-root .nova-skeleton--grid .nova-skeleton__line{height:.8em;margin-bottom:.4em}'+
    '@media screen and (max-width:860px){.nova-wide-root .nova-skeleton--grid .nova-skeleton__row{width:33.3333%}}'+
    '@media screen and (max-width:580px){.nova-wide-root .nova-skeleton--grid .nova-skeleton__row{width:50%}}';

  var BADGE_CSS = [
    '.nova-wide-root .nova__list--grid .nova-card__num{padding:.45em 0 0 .5em;font-size:1em}',
    '.nova-wide-root .nova__list--grid .nova-card__side{top:.45em;right:.95em;text-align:right}',
    '.nova-wide-root .nova__list--grid .nova-card__num>span,.nova-wide-root .nova__list--grid .nova-card__quality{display:inline-block;font-size:1em;font-weight:600;line-height:1.3;letter-spacing:.02em;padding:.1em .42em;-webkit-border-radius:.35em;border-radius:.35em;background:rgba(10,11,17,.62);-webkit-box-shadow:0 .12em .45em rgba(0,0,0,.4);box-shadow:0 .12em .45em rgba(0,0,0,.4);text-shadow:none;color:#fff}',
    '.nova-wide-root .nova__list--grid .nova-card__thumb:not(.nova-card__thumb--loaded) .nova-card__num>span,.nova-wide-root .nova__list--grid .nova-card__thumb--fallback .nova-card__num>span{background:rgba(10,11,17,.62);padding:.1em .42em;-webkit-box-shadow:0 .12em .45em rgba(0,0,0,.4);box-shadow:0 .12em .45em rgba(0,0,0,.4)}'
  ].join('');

  var WIDE_CSS = ".nova-wide-scope .explorer__files-body .scroll__body>.nova-wide-root.nova-wide,.nova-wide{display:-webkit-flex!important;display:-ms-flexbox!important;display:flex!important;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap;-webkit-align-items:flex-start;-ms-flex-align:start;align-items:flex-start;padding:0 0 2em 0}.nova-wide>.nova__hero{-webkit-flex:0 0 41%;-ms-flex:0 0 41%;flex:0 0 41%;width:41%;max-width:41%;padding:0 1.6em 0 0}.nova-wide>.nova__rows{-webkit-flex:1 1 0%;-ms-flex:1 1 0%;flex:1 1 0%;width:59%;min-width:0;padding:.05em 0 0 0}.nova-wide>.nova-wide__strip{-webkit-flex:0 0 100%;-ms-flex:0 0 100%;flex:0 0 100%;width:100%;margin:1.05em 0 0 0;padding:.4em 0 .6em 1.05em;overflow:visible}.nova-wide>.nova-wide__strip--clip{overflow:hidden}.nova-wide .nova__list--row{-webkit-transition:-webkit-transform .25s;transition:transform .25s}.nova-skin-scope .explorer__files-body .scroll__body>.nova-wide-root,.z01-scope .explorer__files-body .scroll__body>.nova-wide-root{display:block!important}.nova-skin-scope .explorer__files-body .scroll__body>.nova-wide-root.nova-wide,.z01-scope .explorer__files-body .scroll__body>.nova-wide-root.nova-wide{display:-webkit-flex!important;display:-ms-flexbox!important;display:flex!important}.nova-wide-scope .explorer__files-body,.nova-wide-scope .explorer__files-body .scroll__content,.nova-wide-scope .explorer__files-body .scroll__body{-webkit-mask-image:none!important;mask-image:none!important}.nova-wide-scope .explorer__files-body .scroll__body{padding-top:.3em;padding-left:.3em}.nova-wide .nova__rows,.nova-wide-root .nova-wide__panel,.nova-wide .nova-wide__group,.nova-wide .nova-wide__row{overflow:visible}body.nova-wide-fade .nova-wide .nova-hero__bg,body.nova-wide-fade .nova-wide .nova-hero__shade{-webkit-mask-image:linear-gradient(90deg,transparent 0,#000 8%,#000 92%,transparent 100%),linear-gradient(180deg,transparent 0,#000 11%,#000 100%);mask-image:linear-gradient(90deg,transparent 0,#000 8%,#000 92%,transparent 100%),linear-gradient(180deg,transparent 0,#000 11%,#000 100%);-webkit-mask-composite:source-in;mask-composite:intersect}body.nova-wide-fade .nova-wide .nova-hero{-webkit-border-radius:0;border-radius:0;background:none}body.nova-wide-fade .nova-wide .nova-hero__body{padding-left:1.6em;padding-right:1.6em}body.nova-wide-fade .nova-wide .nova-hero__progress{left:0;right:0;bottom:0;-webkit-border-radius:0;border-radius:0}body.nova-wide-fade .nova-wide .nova-hero__season{margin-bottom:0}.nova-wide .nova-hero{min-height:0;margin:0;-webkit-border-radius:.95em;border-radius:.95em}.nova-wide .nova-hero__bg{position:relative;top:auto;left:auto;right:auto;bottom:auto;padding-top:58.5%}.nova-wide .nova-hero__bg img{position:absolute;top:0;left:0;width:100%;height:100%}.nova-wide .nova-hero__shade{background:-webkit-linear-gradient(bottom,rgba(10,11,17,.95) 0%,rgba(10,11,17,.56) 48%,rgba(10,11,17,0) 100%);background:linear-gradient(0deg,rgba(10,11,17,.95) 0%,rgba(10,11,17,.56) 48%,rgba(10,11,17,0) 100%)}.nova-wide .nova-hero__body{position:absolute;left:0;right:0;bottom:0;padding:1.05em 1.15em .9em 1.15em;max-width:100%;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-direction:column;-ms-flex-direction:column;flex-direction:column}.nova-wide .nova-hero--compact .nova-hero__body{position:relative;min-height:0;padding:1em 1.1em}.nova-wide .nova-hero__descr{display:none}.nova-wide .nova-hero__title{font-size:1.68em;margin-bottom:.42em;-webkit-line-clamp:1;max-width:72%}.nova-wide .nova-hero__title--logo{width:100%;max-width:100%;text-align:center}.nova-wide .nova-hero__title--logo>img{display:block;margin-left:auto;margin-right:auto;max-height:2.9em;max-width:86%;width:auto;-webkit-filter:drop-shadow(0 .15em .4em rgba(0,0,0,.5));filter:drop-shadow(0 .15em .4em rgba(0,0,0,.5))}.nova-wide .nova-hero__meta{position:absolute;top:.7em;right:.85em;left:auto;bottom:auto;z-index:3;margin:0;font-size:1em;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-wrap:nowrap;-ms-flex-wrap:nowrap;flex-wrap:nowrap;-webkit-align-items:center;-ms-flex-align:center;align-items:center;-webkit-justify-content:flex-end;-ms-flex-pack:end;justify-content:flex-end;max-width:62%}.nova-wide .nova-hero__meta>*{margin:0 0 0 .9em;padding:0;opacity:.92;white-space:nowrap;background:none!important;-webkit-box-shadow:none!important;box-shadow:none!important;text-shadow:0 .08em .28em rgba(0,0,0,.9)}.nova-wide .nova-hero__actions{margin:0;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap;-webkit-align-items:center;-ms-flex-align:center;align-items:center;max-width:100%}.nova-wide .nova-hero__actions>.nova-btn{font-size:1.08em;padding:.56em 1.18em;margin:0 .55em .3em 0}.nova-wide .nova-hero__hint{-webkit-flex:0 1 auto;-ms-flex:0 1 auto;flex:0 1 auto;font-size:.98em;margin:0 0 .28em 0;min-width:0;white-space:nowrap;max-width:52%}.nova-wide .nova-hero__season{margin:.18em 0 0 .05em;font-size:.92em;opacity:.6}.nova-wide .nova-hero__progress{height:.28em}.nova-wide__group{margin:0 0 .72em 0}.nova-wide__label{display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-align-items:baseline;-ms-flex-align:baseline;align-items:baseline;font-size:.9em;letter-spacing:.1em;text-transform:uppercase;opacity:.45;margin:0 0 .38em .1em}.nova-wide__name{min-width:0;overflow:hidden;-o-text-overflow:ellipsis;text-overflow:ellipsis;white-space:nowrap}.nova-wide__total{-webkit-flex:0 0 auto;-ms-flex:0 0 auto;flex:0 0 auto;margin:0 0 0 .9em;letter-spacing:.03em;text-transform:none;white-space:nowrap}.nova-wide__row{display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap}.nova-wide .nova-wide__group--scroll{overflow:hidden}.nova-wide .nova-wide__row--scroll{-webkit-flex-wrap:nowrap;-ms-flex-wrap:none;flex-wrap:nowrap;-webkit-transition:-webkit-transform .25s;transition:transform .25s}.nova-wide .nova-wide__row--scroll>.nova-chip{-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;margin-bottom:0}.nova-wide .nova-wide__row--pair{-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap;-webkit-flex-direction:column;-ms-flex-direction:column;flex-direction:column;-webkit-align-content:flex-start;-ms-flex-line-pack:start;align-content:flex-start}.nova-wide .nova-wide__row--pair>.nova-chip{margin-bottom:.38em}.nova-wide-root .nova-chip{-webkit-border-radius:.52em;border-radius:.52em;padding:.3em .8em;margin:0 .42em .38em 0;max-width:none;-webkit-align-items:center;-ms-flex-align:center;align-items:center;background:rgba(255,255,255,.08);-webkit-box-shadow:inset 0 0 0 .08em rgba(255,255,255,.06);box-shadow:inset 0 0 0 .08em rgba(255,255,255,.06)}.nova-wide-root .nova-chip__stack{display:block;min-width:0}.nova-wide-root .nova-chip__stack .nova-chip__label{display:block;font-size:1em;line-height:1.2;padding:0}.nova-wide-root .nova-chip__sub{display:block;font-size:.72em;line-height:1.2;opacity:.5;white-space:nowrap}.nova-wide-root .nova-card__tag,.nova-wide-root .nova-card__tags>span,.nova-wide-root .nova-card__strip>span,.nova-wide-root .nova-card__head .nova-badge{background:rgba(10,11,17,.62)!important;-webkit-box-shadow:inset 0 0 0 .08em rgba(255,255,255,.08),0 .08em .28em rgba(0,0,0,.3);box-shadow:inset 0 0 0 .08em rgba(255,255,255,.08),0 .08em .28em rgba(0,0,0,.3)}.nova-wide-root .nova-chip.focus .nova-chip__sub{opacity:.65}.nova-wide-root .nova-chip--active{background:rgba(255,255,255,.16);-webkit-box-shadow:inset 0 0 0 .09em rgba(255,255,255,.45);box-shadow:inset 0 0 0 .09em rgba(255,255,255,.45)}.nova-wide-root .nova-chip--active.focus{-webkit-box-shadow:0 .2em .7em rgba(0,0,0,.4);box-shadow:0 .2em .7em rgba(0,0,0,.4)}.nova-wide-root>.nova__rows>.nova-toolbar{margin:0 0 .85em 0;padding-left:.55em}.nova-wide.nova-wide-root>.nova__rows>.nova-drop{position:absolute!important;visibility:hidden;pointer-events:none;margin:0 0 .85em 0;padding-left:1.55em}.nova-wide.nova-wide-root .nova-wide__panel>.nova-drop{visibility:visible;pointer-events:auto}.nova-wide__bar{-webkit-flex-wrap:nowrap;-ms-flex-wrap:nowrap;flex-wrap:nowrap;margin:0 0 .85em 0;overflow:hidden;padding-right:.4em}.nova-wide__bar .nova-toolbar__label{font-size:.9em;margin:0 .48em 0 0;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0}.nova-wide__bar .nova-chip{padding:.35em .75em;margin:0 .75em 0 0;font-size:.95em;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0}.nova-wide__bar .nova-chip__label{font-size:1em}.nova-wide-root .nova__list>.nova-note{position:static!important;box-sizing:border-box;display:block!important;-webkit-flex:0 0 100%!important;-ms-flex:0 0 100%!important;flex:0 0 100%!important;width:100%!important;max-width:100%!important;height:auto!important;min-height:0!important;max-height:none!important;margin:0 0 1em 0!important;padding:.7em 1em!important;overflow:visible!important;align-self:flex-start!important;transform:none!important;font-size:1em!important;line-height:1.25!important;z-index:1!important}.nova-wide-root .nova__list>.nova-note .nova-note__main{display:block!important;position:static!important;max-width:100%!important;width:100%!important;height:auto!important;margin:0!important;padding:0!important}.nova-wide-root .nova__list>.nova-note .nova-note__title{display:block!important;position:static!important;font-size:1.05em!important;line-height:1.25!important;margin:0 0 .35em 0!important;white-space:normal!important;overflow-wrap:anywhere!important}.nova-wide-root .nova__list>.nova-note .nova-note__text{display:block!important;position:static!important;font-size:.9em!important;line-height:1.3!important;margin:0 0 .6em 0!important;white-space:normal!important;overflow-wrap:anywhere!important}.nova-wide-root .nova__list>.nova-note .nova-note__actions{display:flex!important;position:static!important;flex-wrap:wrap!important;align-items:center!important;gap:.45em!important;margin:0!important;height:auto!important}.nova-wide-root .nova__list>.nova-loading,.nova-wide-root .nova__list>.nova-skeleton{-webkit-flex:0 0 100%;-ms-flex:0 0 100%;flex:0 0 100%;width:100%;max-width:100%;min-width:0;margin:0;padding:0 .55em}.nova-wide-root .nova__list>.nova-loading{-webkit-flex:1 1 auto;-ms-flex:1 1 auto;flex:1 1 auto;width:auto;max-width:none;padding:1.4em 1.6em;margin:0 .55em 1.1em .55em}.nova-wide-root .nova__list>.nova-skeleton{display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap}.nova-wide-root .nova__list>.nova-skeleton>.nova-skeleton__row{display:block;-webkit-flex:0 0 20%;-ms-flex:0 0 20%;flex:0 0 20%;width:20%;max-width:20%;margin:0;padding:0 .55em 1.1em .55em;background:none;-webkit-border-radius:0;border-radius:0}.nova-wide-root .nova__list>.nova-skeleton .nova-skeleton__thumb{width:100%;height:auto;padding-top:56.25%;-webkit-border-radius:.55em;border-radius:.55em}.nova-wide-root .nova__list>.nova-skeleton .nova-skeleton__body{padding:.55em 0 0 0}.nova-wide-root .nova__list>.nova-skeleton .nova-skeleton__line{height:.8em;margin-bottom:.45em}.nova-wide-root .nova-wide__pane{display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap;-webkit-align-items:center;-ms-flex-align:center;align-items:center;-webkit-transition:-webkit-transform .25s;transition:transform .25s}.nova-wide-root .nova-wide__hold-spacer{-webkit-flex:0 0 100%;-ms-flex:0 0 100%;flex:0 0 100%;width:100%;visibility:hidden;pointer-events:none}.nova-wide-root .nova-wide__drop--scroll{overflow:hidden}.nova-wide-root .nova-drop{margin:0}.nova-wide-root .nova-wide__panel>.nova-drop{box-sizing:border-box;-webkit-align-items:flex-start;-ms-flex-align:start;align-items:flex-start}.nova-wide-root .nova-wide__pane{-webkit-flex:0 0 100%;-ms-flex:0 0 100%;flex:0 0 100%;min-width:0;width:100%}.nova-wide__panel--swap>.nova-drop{margin:0 0 .2em 0}.nova-wide__panel--overlay{position:relative}.nova-wide__panel--overlay>.nova-wide__group{visibility:hidden}.nova-wide__panel--overlay>.nova-drop{z-index:4}.nova-wide__panel--note-source>.nova-drop{position:static!important;display:flex!important;visibility:visible;pointer-events:auto;width:100%;max-width:100%;height:auto!important;max-height:none;margin:.35em 0 .8em 0;padding-left:1em!important;z-index:auto}.nova-wide .nova__list>.nova-note{position:relative!important;box-sizing:border-box;display:block!important;flex:0 0 100%!important;width:100%!important;max-width:100%!important;min-width:0;min-height:0;margin:0 0 1em 0;padding:.7em 1em;overflow:visible!important;font-size:1em;line-height:1.25;z-index:1}.nova-wide .nova__list>.nova-note .nova-note__main{display:block;max-width:100%;margin:0;padding:0}.nova-wide .nova__list>.nova-note .nova-note__title{font-size:1.05em;line-height:1.25;margin:0 0 .35em 0;white-space:normal;overflow-wrap:anywhere}.nova-wide .nova__list>.nova-note .nova-note__text{font-size:.9em;line-height:1.3;margin:0 0 .6em 0;white-space:normal;overflow-wrap:anywhere}.nova-wide .nova__list>.nova-note .nova-note__actions{display:flex;flex-wrap:wrap;align-items:center;gap:.45em;margin:0}.nova-wide .nova__list>.nova-note .nova-btn{font-size:.95em;line-height:1.2;white-space:nowrap}.nova-wide-root .nova-note{position:relative!important;display:block!important;inset:auto!important;box-sizing:border-box!important;width:100%!important;max-width:100%!important;height:auto!important;min-height:0!important;max-height:none!important;margin:0 0 1em 0!important;padding:.7em 1em!important;overflow:visible!important;transform:none!important;font-size:1em!important;line-height:1.25!important;z-index:1!important}.nova-wide-root .nova-note__main{position:static!important;display:block!important;width:100%!important;max-width:100%!important;height:auto!important;margin:0!important;padding:0!important}.nova-wide-root .nova-note__title{position:static!important;display:block!important;font-size:1.05em!important;line-height:1.25!important;margin:0 0 .35em 0!important;white-space:normal!important;overflow-wrap:anywhere!important}.nova-wide-root .nova-note__text{position:static!important;display:block!important;font-size:.9em!important;line-height:1.3!important;margin:0 0 .6em 0!important;white-space:normal!important;overflow-wrap:anywhere!important}.nova-wide-root .nova-note__actions{position:static!important;display:flex!important;flex-wrap:wrap!important;align-items:center!important;gap:.45em!important;height:auto!important;margin:0!important}.nova-wide-root:has(.nova-wide__panel--note-source) .nova-note,.nova-wide-root.nova-wide--note-source .nova-note{display:none!important}.nova-wide-root:not(.nova-wide)>.nova__list{display:block!important;position:static!important;box-sizing:border-box!important;-webkit-flex:0 0 100%!important;-ms-flex:0 0 100%!important;flex:0 0 100%!important;width:100%!important;max-width:100%!important;min-width:0!important;margin:1.05em 0 0 0!important;padding:0!important;transform:none!important;overflow:visible!important}.nova-wide-root:not(.nova-wide)>.nova__list--grid:not(.nova__list--row){display:-webkit-flex!important;display:-ms-flexbox!important;display:flex!important;-webkit-flex-wrap:wrap!important;-ms-flex-wrap:wrap!important;flex-wrap:wrap!important;-webkit-align-items:flex-start!important;-ms-flex-align:start!important;align-items:flex-start!important;margin:1.05em -.45em 0 -.45em!important}.nova-wide-root:not(.nova-wide)>.nova__list>.nova-note{display:block!important;position:static!important;float:none!important;width:100%!important;max-width:100%!important;margin:0 0 1em 0!important}:is(.nova-skin-root,.nova-wide-root)>.nova__rows>.nova-drop{position:static!important;display:flex!important;visibility:visible!important;pointer-events:auto!important;width:100%!important;max-width:100%!important;height:auto!important;max-height:none!important;margin:0 0 .8em 0!important;padding-left:1em!important;z-index:auto!important}:is(.nova-skin-root,.nova-wide-root) .nova__list>.nova-note{position:static!important;box-sizing:border-box;display:block!important;flex:0 0 100%!important;width:100%!important;max-width:100%!important;height:auto!important;min-height:0!important;max-height:none!important;margin:0 0 1em 0!important;padding:.7em 1em!important;overflow:visible!important;transform:none!important;font-size:1em!important;line-height:1.25!important;z-index:1!important}:is(.nova-skin-root,.nova-wide-root) .nova__list>.nova-note .nova-note__main{display:block!important;position:static!important;width:100%!important;max-width:100%!important;height:auto!important;margin:0!important;padding:0!important}:is(.nova-skin-root,.nova-wide-root) .nova__list>.nova-note .nova-note__title{display:block!important;position:static!important;font-size:1.05em!important;line-height:1.25!important;margin:0 0 .35em 0!important;white-space:normal!important;overflow-wrap:anywhere!important}:is(.nova-skin-root,.nova-wide-root) .nova__list>.nova-note .nova-note__text{display:block!important;position:static!important;font-size:.9em!important;line-height:1.3!important;margin:0 0 .6em 0!important;white-space:normal!important;overflow-wrap:anywhere!important}:is(.nova-skin-root,.nova-wide-root) .nova__list>.nova-note .nova-note__actions{display:flex!important;position:static!important;flex-wrap:wrap!important;align-items:center!important;gap:.45em!important;margin:0!important;height:auto!important}:is(.nova-skin-root,.nova-wide-root) .nova__list>.nova-note .nova-btn{font-size:.95em!important;line-height:1.2!important;white-space:nowrap!important}.nova-wide__panel--bottom>.nova-wide__bar{margin:.35em 0 0 0}.nova-wide__panel--bottom>.nova-wide__group:last-of-type{margin-bottom:0}body:not(.nova-wide-focus-ring) .nova-wide-root .nova-chip.focus{background:#fff!important;color:#000!important;-webkit-box-shadow:none!important;box-shadow:none!important}body:not(.nova-wide-focus-ring) .nova-wide-root .nova-chip.focus .nova-chip__sub{opacity:.6}body:not(.nova-wide-focus-ring) .nova-wide-root .nova-chip.focus .nova-chip__badge{background:rgba(0,0,0,.12)!important;color:#000!important}body:not(.nova-wide-focus-ring) .nova-wide-root .nova-card--wide.focus .nova-card__body{background:#fff;color:#000;-webkit-border-radius:.45em;border-radius:.45em;padding:.3em .45em;margin:-.3em -.45em}body:not(.nova-wide-focus-ring) .nova-wide-root .nova-card--wide.focus .nova-card__descr,body:not(.nova-wide-focus-ring) .nova-wide-root .nova-card--wide.focus .nova-card__voice{opacity:.75}body:not(.nova-wide-focus-ring) .nova-wide-root .nova-card--wide.focus .nova-card__head .nova-badge{background:rgba(0,0,0,.12)!important;color:#000!important;-webkit-box-shadow:none!important;box-shadow:none!important}body:not(.nova-wide-focus-ring) .nova-wide-root .nova-card--wide.focus .nova-card__tags>span{background:rgba(0,0,0,.1)!important;-webkit-box-shadow:none!important;box-shadow:none!important}body:not(.nova-wide-focus-ring) .nova-wide-root .nova-card--wide.focus .nova-card__thumb{-webkit-box-shadow:0 0 0 .16em #fff;box-shadow:0 0 0 .16em #fff}.nova-wide-root .nova__list{display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap;margin:0 -.55em;-webkit-transition:-webkit-transform .25s;transition:transform .25s}.nova-wide-root>.nova__list{padding:.4em 0 .6em .55em}.nova-wide-root .nova__list--row{-webkit-flex-wrap:nowrap;-ms-flex-wrap:none;flex-wrap:nowrap}.nova-wide-root .nova-card--wide{display:block;-webkit-flex:0 0 20%;-ms-flex:0 0 20%;flex:0 0 20%;width:20%;max-width:20%;margin:0;padding:0 .55em 1.1em .55em;background:none}.nova-wide-root .nova-card--wide.focus{background:none;color:inherit}.nova-wide-root .nova-card--wide .nova-card__thumb{position:relative;width:100%;height:auto;margin:0;padding-top:56.25%;overflow:hidden;-webkit-border-radius:.55em;border-radius:.55em;background:rgba(255,255,255,.07)}.nova-wide-root .nova-card--wide .nova-card__thumb img{position:absolute;top:0;left:0;width:100%;height:100%;-o-object-fit:cover;object-fit:cover}.nova-wide-root .nova-card--wide.focus .nova-card__thumb{-webkit-box-shadow:0 0 0 .16em #fff;box-shadow:0 0 0 .16em #fff}.nova-wide-root .nova-card--wide.focus .nova-card__thumb:after{content:\"\\25B6\";position:absolute;top:50%;left:50%;width:2em;height:2em;margin:-1em 0 0 -1em;z-index:3;-webkit-border-radius:50%;border-radius:50%;background:rgba(16,18,26,.55);color:#fff;font-size:1.05em;line-height:2em;text-align:center}.nova-wide-root .nova-card__tag{position:absolute;top:.4em;left:.45em;z-index:3;padding:.12em .4em;-webkit-border-radius:.3em;border-radius:.3em;font-size:.85em;font-weight:600;line-height:1.4}.nova-wide-root .nova-card__strip{position:absolute;left:.55em;right:.55em;bottom:.7em;z-index:2;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-justify-content:space-between;-ms-flex-pack:justify;justify-content:space-between;font-size:.8em;text-shadow:0 .06em .2em rgba(0,0,0,.75)}.nova-wide-root .nova-card__pct{font-weight:600}.nova-wide-root .nova-card__strip>span{padding:.1em .38em;-webkit-border-radius:.3em;border-radius:.3em;line-height:1.45;text-shadow:none}.nova-wide-root .nova-chip--stack{-webkit-align-items:flex-start;-ms-flex-align:start;align-items:flex-start}.nova-wide-root .nova-drop .nova-chip__badge{background:rgba(255,255,255,.2);-webkit-box-shadow:none;box-shadow:none;-webkit-align-self:center;-ms-flex-item-align:center;align-self:center}.nova-wide-root .nova-chip__badge{-webkit-align-self:center;-ms-flex-item-align:center;align-self:center;margin-top:0}.nova-wide-root .nova-card--wide .nova-card__line{position:absolute;left:0;right:0;bottom:0;z-index:2;height:.22em;margin:0;background:rgba(255,255,255,.25)}.nova-wide-root .nova-card--wide .nova-card__body{padding:0}.nova-wide-root .nova-card__head{display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-align-items:center;-ms-flex-align:center;align-items:center;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap;margin:.5em 0 .25em 0;font-size:.85em;opacity:.7}.nova-wide-root .nova-card__head>*{margin:0 .5em .1em 0}.nova-wide-root .nova-card__head .nova-badge{font-size:.85em;opacity:1}.nova-wide-root .nova-card__rate{font-weight:600;opacity:1}.nova-wide-root .nova-card__voice{min-width:0;max-width:100%;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;white-space:normal;word-break:break-word;opacity:.9}.nova-wide-root .nova-card--wide .nova-card__title{margin:0 0 .3em 0;font-size:1.05em;line-height:1.3;overflow:hidden;display:-webkit-box;-webkit-line-clamp:1;-webkit-box-orient:vertical}.nova-wide-root .nova-card__tags{display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap;margin:0 0 .35em 0}.nova-wide-root .nova-card__tags>span{margin:0 .3em .25em 0;padding:.1em .35em;-webkit-border-radius:.25em;border-radius:.25em;font-size:.72em;letter-spacing:.03em;line-height:1.5}.nova-wide-root .nova-card__tags>.nova-card__voice{background:none!important;-webkit-box-shadow:none!important;box-shadow:none!important;padding:.1em 0;margin:0 .3em .25em .15em;font-size:.82em;letter-spacing:0;line-height:1.5;opacity:.7}.nova-wide-root .nova-card--wide.nova-card--file .nova-card__thumb{width:100%;height:auto;padding-top:56.25%}.nova-wide-root .nova-card--wide.nova-card--file .nova-card__body{padding:0}.nova-wide-root .nova-card__descr{font-size:.85em;line-height:1.35;opacity:.5;overflow:hidden;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical}.nova-wide-root .nova-card--wide .nova-card__meta{font-size:.8em;opacity:.5}.nova-wide-root .nova-card__eye{position:absolute;top:.42em;right:.42em;left:auto;bottom:auto;z-index:3;width:1.28em;height:1.28em;padding:.12em;-webkit-border-radius:.32em;border-radius:.32em;background:rgba(10,11,17,.62);-webkit-box-shadow:inset 0 0 0 .08em rgba(255,255,255,.08),0 .08em .28em rgba(0,0,0,.25);box-shadow:inset 0 0 0 .08em rgba(255,255,255,.08),0 .08em .28em rgba(0,0,0,.25)}.nova-wide-root .nova-card__eye svg,.nova-wide-root .nova-card__viewed svg{width:100%;height:100%;display:block}.nova-wide-root .nova-card__viewed{top:.42em;right:.42em;left:auto;bottom:auto;z-index:3;width:1.28em;height:1.28em;padding:.12em;-webkit-border-radius:.32em;border-radius:.32em;background:rgba(10,11,17,.62);-webkit-box-shadow:inset 0 0 0 .08em rgba(255,255,255,.08),0 .08em .28em rgba(0,0,0,.25);box-shadow:inset 0 0 0 .08em rgba(255,255,255,.08),0 .08em .28em rgba(0,0,0,.25)}.nova-wide-root .nova-card--wide-nav,.nova-wide-root .nova-card--nav{-webkit-flex:0 0 100%;-ms-flex:0 0 100%;flex:0 0 100%;width:100%;max-width:100%;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-align-items:center;-ms-flex-align:center;align-items:center}.nova-wide-root .nova-card--wide-nav .nova-card__thumb,.nova-wide-root .nova-card--nav .nova-card__thumb{-webkit-flex:0 0 7.2em;-ms-flex:0 0 7.2em;flex:0 0 7.2em;width:7.2em;height:4.05em;padding-top:0}.nova-wide-root .nova-card--wide-nav .nova-card__body,.nova-wide-root .nova-card--nav .nova-card__body{-webkit-flex:1 1 0%;-ms-flex:1 1 0%;flex:1 1 0%;min-width:0;padding:0 .8em}.nova-wide-root .nova-card--wide-nav .nova-card__title,.nova-wide-root .nova-card--nav .nova-card__title{-webkit-line-clamp:2}@media screen and (min-aspect-ratio:5/4) and (max-width:1200px){.nova-wide-root .nova-card--wide{-webkit-flex:0 0 25%;-ms-flex:0 0 25%;flex:0 0 25%;width:25%;max-width:25%}.nova-wide-root .nova-card__descr{-webkit-line-clamp:2}}@media screen and (min-aspect-ratio:5/4) and (max-width:820px){.nova-wide-root .nova-card--wide{-webkit-flex:0 0 33.3333%;-ms-flex:0 0 33.3333%;flex:0 0 33.3333%;width:33.3333%;max-width:33.3333%}}@media screen and (max-width:1100px) and (max-aspect-ratio:5/4){.nova-wide>.nova__hero,.nova-wide>.nova__rows{-webkit-flex:0 0 100%;-ms-flex:0 0 100%;flex:0 0 100%;width:100%;max-width:100%;padding:0}.nova-wide>.nova__rows{margin-top:1em}.nova-wide>.nova-wide__strip{padding-left:0}.nova-wide-root .nova__list{margin-left:0;margin-right:0}.nova-wide-root>.nova__list{padding-left:0;padding-right:0}.nova-wide__bar{-webkit-flex-wrap:wrap;-ms-flex-wrap:wrap;flex-wrap:wrap}.nova-wide-root .nova-card--wide{-webkit-flex:0 0 33.3333%;-ms-flex:0 0 33.3333%;flex:0 0 33.3333%;width:33.3333%;max-width:33.3333%}}@media screen and (max-width:640px) and (max-aspect-ratio:5/4){.nova-wide-root .nova-card--wide{-webkit-flex:0 0 50%;-ms-flex:0 0 50%;flex:0 0 50%;width:50%;max-width:50%}.nova-wide-root .nova-card__descr{display:none}.nova-wide .nova-hero__title{max-width:100%}.nova-wide .nova-hero__hint{max-width:100%;-webkit-flex-basis:100%;-ms-flex-preferred-size:100%;flex-basis:100%}}";
  ...
  var EXTRA_CSS = ".nova-wide-root .nova-btn>svg{width:1.15em!important;height:1.15em!important;max-width:1.15em;max-height:1.15em;-webkit-box-flex:0;-webkit-flex:0 0 auto;-ms-flex:0 0 auto;flex:0 0 auto}.nova-wide-root .nova-chip>svg{width:1em!important;height:1em!important;max-width:1em;max-height:1em;-webkit-box-flex:0;-webkit-flex:0 0 auto;-ms-flex:0 0 auto;flex:0 0 auto}.nova-wide-root .nova-card__eye>svg{width:1.2em!important;height:1.2em!important}.nova-wide-root .nova-card__viewed>svg{width:100%!important;height:100%!important}.nova-wide-root .nova-btn__label{min-width:0;overflow:hidden;-o-text-overflow:ellipsis;text-overflow:ellipsis}.nova-wide-chips .explorer__files-head{display:none!important}.nova-wide-scope .nova-wide-hidden{display:none!important}.nova-wide-scope .nova{padding-top:0}.nova-wide-root .nova-card__line>.time-line{display:block!important;height:100%;margin:0;background:none}.nova-wide-root .nova-card__line>.time-line>div{height:100%;background:#fff}.nova-wide-root .nova-card__line--body>.time-line>div{background:rgba(255,255,255,.9)}.nova-wide-root .nova-hero__progress .time-line{display:block!important;height:100%;margin:0;background:none}.nova-wide-root .nova-hero__progress .time-line>div{height:100%;background:#fff}.nova-wide-scope .explorer__files-body .scroll__body>*{display:none!important}.nova-wide-scope .explorer__files-body .scroll__body>.nova-wide-root{display:block!important}.nova-wide-scope .explorer__files-body .broadcast__scan{display:none!important}.nova-wide-scope .explorer__files-head{display:none!important}.nova-wide-switching .activity--active{-webkit-animation:none!important;animation:none!important;-webkit-transition:none!important;transition:none!important;-webkit-transform:none!important;transform:none!important}.nova-wide-switching .activity:not(.activity--active){opacity:0!important}.nova-wide-switching .explorer__left,.nova-wide-switching .explorer__files{-webkit-transition:none!important;transition:none!important;-webkit-animation:none!important;animation:none!important}.nova-wide-root .nova-toolbar .nova-chip{-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;max-width:none}.nova-wide-root .nova-toolbar--clip .nova-chip{-webkit-flex-shrink:1;-ms-flex-negative:1;flex-shrink:1;min-width:4.5em}.nova-wide-root .nova-toolbar--clip .nova-chip__label{overflow:hidden;-o-text-overflow:ellipsis;text-overflow:ellipsis;min-width:0}.nova-wide-root .nova-hero__title,.nova-wide-root .nova-hero__meta,.nova-wide-root .nova-hero__descr,.nova-wide-root .nova-hero__hint,.nova-wide-root .nova-hero__season{text-shadow:0 .06em .5em rgba(0,0,0,.8)}.nova-wide-root .nova-hero__descr{opacity:.8}.nova-wide-root .nova-hero__title--logo{display:block;overflow:visible;-webkit-line-clamp:none;-webkit-box-orient:horizontal;padding:.08em 0 .06em;margin-bottom:.5em;line-height:1}.nova-wide-root .nova-hero__title--logo>img{display:block;max-height:2.1em;max-width:70%;width:auto;height:auto;-o-object-fit:contain;object-fit:contain;image-rendering:-webkit-optimize-contrast;-webkit-filter:drop-shadow(0 .04em .12em rgba(0,0,0,.55));filter:drop-shadow(0 .04em .12em rgba(0,0,0,.55))}.nova-wide-root .nova-hero__title--logo>img.nova-logo--invert{-webkit-filter:invert(1) brightness(1.1) drop-shadow(0 .04em .12em rgba(0,0,0,.5));filter:invert(1) brightness(1.1) drop-shadow(0 .04em .12em rgba(0,0,0,.5))}.nova-wide-root .nova-hero__title--logo>img.nova-logo--glow{-webkit-filter:drop-shadow(0 0 .03em rgba(255,255,255,.5)) drop-shadow(0 0 .07em rgba(255,255,255,.35));filter:drop-shadow(0 0 .03em rgba(255,255,255,.5)) drop-shadow(0 0 .07em rgba(255,255,255,.35))}@media screen and (max-width:580px){.nova-wide-root .nova-hero__title--logo>img{max-height:1.9em;max-width:80%}}.nova-wide-root .nova-chip--checking{opacity:.55}.nova-wide-root .nova-chip--checking .nova-chip__label{opacity:.6}@media screen and (max-width:900px){.nova-wide-root .nova-hero__hint{-webkit-flex:1 0 100%;-ms-flex:1 0 100%;flex:1 0 100%;margin:.3em 0 0 .15em;white-space:normal}}@media screen and (max-width:580px){.nova-wide-root .nova-card__side{display:block!important;text-align:center;padding-right:.2em;max-width:6em;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0}.nova-wide-root .nova-card__quality{font-size:.66em;padding:.15em .4em}.nova-wide-root .nova-card__time{font-size:.78em;margin-top:.3em;display:block}.nova-wide-root .nova__list--grid .nova-card__side{position:absolute;top:.4em;right:.6em;max-width:none}.nova-wide-root .nova__list--grid .nova-card__time{display:none}}" +
     ".nova-wide-root .nova-card__go{margin-left:auto;padding-right:.5em;-webkit-align-self:center;-ms-flex-item-align:center;align-self:center}" +
     ".nova-wide-root .nova__list--grid .nova-card__go{display:none}" +
     ".nova-wide-root .nova-card--nav .nova-card__thumb--poster{width:4.7em;height:7em;background:rgba(0,0,0,.5)}" +
     ".nova-wide-root .nova-card--nav .nova-card__thumb--poster img{-o-object-fit:contain;object-fit:contain}" +
     ".nova-wide-root .nova__list--grid .nova-card--nav .nova-card__thumb--poster{width:100%;height:auto;padding-top:150%}" +
     "@media screen and (max-width:580px){.nova-wide-root .nova-card--nav .nova-card__thumb--poster{width:3.7em;height:5.5em}}";

  var FOCUS_CSS = [
    'body.nova-wide-focus-ring .nova-btn.focus,body.nova-wide-focus-ring .nova-chip.focus,body.nova-wide-focus-ring .nova-card.focus,body.nova-wide-focus-ring .nova-group.focus{background:rgba(255,255,255,.16)!important;color:#fff!important;-webkit-box-shadow:inset 0 0 0 .12em #fff!important;box-shadow:inset 0 0 0 .12em #fff!important}',
    'body.nova-wide-focus-ring .nova-btn--main{background:rgba(255,255,255,.16);color:#fff;-webkit-box-shadow:inset 0 0 0 .1em rgba(255,255,255,.5);box-shadow:inset 0 0 0 .1em rgba(255,255,255,.5)}',
    'body.nova-wide-focus-ring .nova-chip.focus .nova-chip__badge{background:rgba(255,255,255,.2)!important;color:#fff!important}',
    'body.nova-wide-focus-ring .nova-card.focus .nova-card__line--body{background:rgba(255,255,255,.2)!important}',
    'body.nova-wide-focus-ring .nova-card.focus .nova-card__line--body .time-line>div{background:#fff!important}',
    'body.nova-wide-focus-ring .nova-card--slim.focus .nova-card__line{background:rgba(255,255,255,.2)!important}',
    'body.nova-wide-focus-ring .nova-card--slim.focus .nova-card__line .time-line>div{background:#fff!important}',
    'body.nova-wide-focus-ring .nova__list--grid .nova-card.focus{background:none!important;color:inherit!important;-webkit-box-shadow:none!important;box-shadow:none!important}',
    'body.nova-wide-focus-ring .nova__list--grid .nova-card.focus .nova-card__thumb{-webkit-box-shadow:0 0 0 .12em #fff!important;box-shadow:0 0 0 .12em #fff!important}',
    'body.nova-wide-focus-ring .nova-chip--active.focus,body.nova-wide-focus-ring .nova-group--open.focus{-webkit-box-shadow:inset 0 0 0 .12em #fff!important;box-shadow:inset 0 0 0 .12em #fff!important}'
  ].join('');

  var CARD_CSS = [
    '.nova-wide-root .nova-card__viewed{top:auto;bottom:.55em;left:.55em;width:1.15em;height:1.15em;-webkit-border-radius:0;border-radius:0;background:none;opacity:.8;-webkit-box-shadow:none;box-shadow:none}',
    '.nova-wide-root .nova-card__viewed>svg{display:block;width:100%;height:100%;-webkit-filter:drop-shadow(0 0 .2em rgba(0,0,0,.9));filter:drop-shadow(0 0 .2em rgba(0,0,0,.9))}',
    '.nova-wide-root .nova-card--soon{opacity:.45}',
    '.nova-wide-root .nova-card--soon .nova-card__side{max-width:8em;min-width:0;padding-right:.35em;overflow:hidden}',
    '.nova-wide-root .nova-card--soon .nova-card__time{white-space:normal;overflow-wrap:break-word;word-wrap:break-word;word-break:break-word;line-height:1.25;max-width:100%}',
    '@media screen and (max-width:580px){.nova-wide-root .nova-card--soon .nova-card__side{max-width:6em;padding-right:.1em}.nova-wide-root .nova-card--soon .nova-card__time{font-size:.72em}}',
    '@media screen and (max-width:420px){.nova-wide-root .nova-card--soon .nova-card__side{max-width:5em}.nova-wide-root .nova-card--soon .nova-card__time{font-size:.68em}}',
    '.nova-wide-root .nova-card__eye{display:block;margin-top:.4em;opacity:.5}',
    '.nova-wide-root .nova-card__eye>svg{display:block;width:1.2em;height:1.2em;margin:0 auto}',
    '.nova-wide-root .nova-card.focus .nova-card__eye{opacity:.65}',
    'body.nova-wide-focus-ring .nova-card.focus .nova-card__eye{opacity:.75!important}'
  ].join('');

  var MARK_CSS = [
    '.nova-wide-root .nova-hero__mark{display:-webkit-box;display:-webkit-flex;display:-ms-flexbox;display:flex;-webkit-box-align:center;-webkit-align-items:center;-ms-flex-align:center;align-items:center;-webkit-flex-shrink:0;-ms-flex-negative:0;flex-shrink:0;min-width:0;max-width:40%;margin:0 1.2em .4em 0;font-size:1.2em;font-weight:600;line-height:1.25;overflow:hidden;white-space:nowrap;-o-text-overflow:ellipsis;text-overflow:ellipsis}',
    '.nova-wide-root .nova-hero__mark:empty{display:none}',
    '.nova-wide-root .nova-hero__mark.nova-hero__title--logo{display:block;overflow:visible;padding:.08em 0 .06em;margin-bottom:.4em;line-height:1}',
    '.nova-wide-root .nova-hero__mark>img{max-height:1.75em!important;max-width:100%!important;width:auto;height:auto}',
    '@media screen and (max-width:900px){.nova-wide-root .nova-hero__mark{max-width:55%;font-size:1.05em}}',
    '@media screen and (max-width:580px){.nova-wide-root .nova-hero__mark{-webkit-box-ordinal-group:2;-webkit-order:1;-ms-flex-order:1;order:1;max-width:100%;margin-right:0}.nova-wide-root .nova-hero__mark>img{max-height:1.5em!important}}'
  ].join('');

  var FULL_CSS = [
    'body.nova-wide-full .nova-wide-scope .explorer__left{display:none!important}',
    'body.nova-wide-full .nova-wide-scope .explorer__files{width:100%!important;left:0!important}'
  ].join('');
  ...
  var FADE_CSS = [
    'body.nova-wide-fade .nova-wide-root .nova-hero{background:transparent;-webkit-border-radius:0;border-radius:0}',
    'body.nova-wide-fade .nova-wide-root .nova-hero__progress{left:2.2em;right:2.2em;bottom:1.5em;width:auto;-webkit-border-radius:.3em;border-radius:.3em}',
    'body.nova-wide-fade .nova-wide-root .nova-hero--compact .nova-hero__progress{left:1.4em;right:1.4em;bottom:.9em}',
    '@media screen and (max-width:580px){body.nova-wide-fade .nova-wide-root .nova-hero__progress{left:1.3em;right:1.3em;bottom:1em}}',
    'body.nova-wide-fade .nova-wide-root .nova-hero__season{margin-bottom:.6em}',
    'body.nova-wide-fade .nova-wide-root .nova-hero--compact .nova-hero__season{margin-bottom:1em}',
    '@media screen and (max-width:580px){body.nova-wide-fade .nova-wide-root .nova-hero__season{margin-bottom:.9em}}',
    'body.nova-wide-fade .nova-wide-root .nova-hero__bg{' + FADE_MASK + '}',
    'body.nova-wide-fade .nova-wide-root .nova-hero__shade{' + FADE_MASK + '}'
  ].join('');
