(function () {
  ...
  function wideDragLimit(box, axis) {
    try {
      ...
      return Math.min(0, box[0].getBoundingClientRect().height - pane[0].scrollHeight);
    } catch (e) { return 0; }
  }
  ...
  function wideFit() {
    ...
    var hero = 0;
    try {
      hero = ui.hero_box[0].getBoundingClientRect().height || 0;
      panel.find('.nova-wide__group').removeClass('nova-wide__group--scroll');
      rows.removeClass('nova-wide__row--scroll nova-wide__row--pair').each(function () { $(this).css('height', ''); wideRowShift($(this), 0); });
    } catch (e) { return; }
    if (!hero) return;
  ...
  function wideDropFit(drop) {
    ...
    try {
      ...
      var room;
      if (box.top >= hero.bottom - 1) {
        room = view ? Math.floor(view - box.top) - 24 : tall;
      } else {
        room = Math.floor(hero.bottom - box.top) - 12;
        if (room < Math.round(hero.height * 0.25)) room = Math.round(hero.height * 0.25);
      }
      if (view && room > view - box.top - 24) room = Math.floor(view - box.top) - 24;
  ...
  function widePaneFollow(node) {
    ...
    try {
      ...
      var pad = 10;
      var limit = Math.min(0, frame.height - pane[0].scrollHeight);
      var next = shift;
